    ...
    <div className="min-h-screen bg-background">
      <section className="relative bg-gradient-to-br from-background to-muted py-12 lg:py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link href="/syro/directory" className="font-body text-primary hover:underline mb-4 inline-block">
            ← Directory
          </Link>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {entries.length === 0 ? (
            <div className="bg-muted/20 rounded-lg p-8 text-center">
              ...
              <Link href="/syro/directory" className="font-body text-primary font-medium mt-4 inline-block hover:underline">
                Back to Directory
              </Link>
            </div>
          ) : (
...
function EntryCard({ entry }: { entry: DirectoryEntry }) {
  return (
    <li className="bg-muted/20 rounded-lg overflow-hidden sacred-shadow-sm border-l-4 border-primary hover:sacred-shadow reverent-transition">
      <Link href={`/syro/directory/entry/${entry.documentId}`} className="flex gap-4 p-6 group">
        {entry.imageUrl && (
