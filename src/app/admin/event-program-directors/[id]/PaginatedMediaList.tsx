...
export default function PaginatedMediaList({
  ...
  const loadMedia = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const allMedia = await fetchDirectorMediaServer(directorId);
      const startIndex = currentPage * pageSize;
      const endIndex = startIndex + pageSize;
      const paginatedMedia = allMedia.slice(startIndex, endIndex);
      setMediaList(paginatedMedia);
    } catch (error) {
      console.error('Failed to fetch director media:', error);
      setMediaList(initialMediaList);
    } finally {
      setLoading(false);
    }
  }, [directorId, currentPage, pageSize, initialMediaList]);
    ...
    <div>
      {mediaList.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">No media files found for this director.</p>
        </div>
      ) : (
