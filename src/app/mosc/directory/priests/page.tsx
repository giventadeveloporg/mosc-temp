...
export default async function PriestsPage({ searchParams }: PageProps) {
  ...
  const { priests, pagination } = await getPriestsData({ page, pageSize: PAGE_SIZE });

  return (
    <div className="min-h-screen bg-background">
      <section className="relative bg-gradient-to-br from-background to-muted py-12 lg:py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link href="/mosc/directory" className="font-body text-primary hover:underline mb-4 inline-block">← Directory</Link>
          <h1 className="font-heading font-semibold text-3xl lg:text-4xl text-foreground">Priests</h1>
          <p className="font-body text-muted-foreground mt-2">{pagination.total} priest{pagination.total !== 1 ? 's' : ''}. Data from the directory API.</p>
        </div>
      </section>
              ...
              {pagination.pageCount > 1 && (
                <div className="mt-8 flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex gap-3">
                    {pagination.page > 1 && (
                      <Link href={`/mosc/directory/priests?page=${pagination.page - 1}`} className="px-4 py-2 bg-primary/10 text-primary font-body font-medium rounded-lg hover:bg-primary/20 reverent-transition">Previous</Link>
                    )}
                    {pagination.page < pagination.pageCount && (
                      <Link href={`/mosc/directory/priests?page=${pagination.page + 1}`} className="px-4 py-2 bg-primary/10 text-primary font-body font-medium rounded-lg hover:bg-primary/20 reverent-transition">Next</Link>
                    )}
                  </div>
                </div>
              )}
...
function PriestCard({ priest }: { priest: Priest }) {
  return (
    <li className="bg-muted/20 rounded-lg p-6 sacred-shadow-sm border-l-4 border-primary hover:sacred-shadow reverent-transition">
      <Link href={`/mosc/directory/priests/${priest.documentId}`} className="flex gap-4 group">
        {priest.imageUrl && (
