    ...
    if (stripeCustomerId && !existingSubscription) {
      try {
        ...
        const stripeSubscriptions = await stripe().subscriptions.list({
          ...
          limit: 10, // Get up to 10 subscriptions (should be rare to have more than 1)
        });

        console.log('[MEMBERSHIP-SUCCESS] Stripe API subscriptions found:', {
          customerId: stripeCustomerId,
          count: stripeSubscriptions.data.length,
          subscriptionIds: stripeSubscriptions.data.map(sub => sub.id),
          statuses: stripeSubscriptions.data.map(sub => sub.status),
        });
        ...
        if (activeStripeSubscriptions.length > 0) {
          console.log('[MEMBERSHIP-SUCCESS] ⚠️ Found active Stripe subscriptions - will cancel before creating new one:', {
            count: activeStripeSubscriptions.length,
            subscriptionIds: activeStripeSubscriptions.map(sub => sub.id),
            note: 'Database may be out of sync - Stripe API is source of truth',
          });

          // Cancel all active Stripe subscriptions
          for (const stripeSub of activeStripeSubscriptions) {
            try {
              // Check if subscription is for a different plan (plan switch scenario)
              const stripePlanId = stripeSub.items.data[0]?.price?.id;
              const isPlanSwitch = stripePlanId !== finalStripePriceId;

              if (isPlanSwitch) {
                console.log('[MEMBERSHIP-SUCCESS] Plan switch detected in Stripe - canceling old subscription:', {
                  oldSubscriptionId: stripeSub.id,
                  oldPlanId: stripePlanId,
                  newPlanId: finalStripePriceId,
                });
              }

              // Cancel the Stripe subscription (cancel at period end to maintain access)
              await stripe().subscriptions.update(stripeSub.id, {
                cancel_at_period_end: true,
                metadata: {
                  ...stripeSub.metadata,
                  cancellation_reason: isPlanSwitch
                    ? `Switched to plan ${membershipPlanId} (Stripe API sync)`
                    : `Replaced by new subscription (Stripe API sync)`,
                  cancelled_at: new Date().toISOString(),
                  cancelled_by: 'processMembershipSubscriptionFromPaymentIntent',
                },
              });

              console.log('[MEMBERSHIP-SUCCESS] ✅ Cancelled active Stripe subscription:', {
                subscriptionId: stripeSub.id,
                status: stripeSub.status,
                cancelAtPeriodEnd: true,
              });

              // Also update database if subscription exists there
              try {
                // Try to find subscription in database by Stripe subscription ID
                const tenantId = getTenantId();
                const params = new URLSearchParams({
                ...
                const dbSubRes = await fetchWithJwtRetry(
                  `${getAppUrl()}/api/proxy/membership-subscriptions?${params.toString()}`,
                  { cache: 'no-store' }
                );

                if (dbSubRes.ok) {
                  const dbSubs: MembershipSubscriptionDTO[] = await dbSubRes.json();
                  if (dbSubs.length > 0) {
                    const dbSub = dbSubs[0];
                    const cancelPayload = withTenantId({
                      id: dbSub.id!,
                      cancelAtPeriodEnd: true,
                      cancellationReason: isPlanSwitch
                        ? `Switched to plan ${membershipPlanId} (Stripe API sync)`
                        : `Replaced by new subscription (Stripe API sync)`,
                      subscriptionStatus: 'CANCELLED',
                      cancelledAt: new Date().toISOString(),
                    ...
                    await fetchWithJwtRetry(
                      `${getAppUrl()}/api/proxy/membership-subscriptions/${dbSub.id}`,
                      {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/merge-patch+json' },
                        body: JSON.stringify(cancelPayload),
                      ...
                      '[MEMBERSHIP-SUCCESS] cancel-stripe-synced-subscription'
                    );

                    console.log('[MEMBERSHIP-SUCCESS] ✅ Updated database subscription to CANCELLED:', dbSub.id);
                  }
                }
              } catch (dbUpdateError) {
                console.warn('[MEMBERSHIP-SUCCESS] ⚠️ Could not update database subscription (non-fatal):', dbUpdateError);
                // Continue - Stripe subscription is cancelled, which is the source of truth
              }
            } catch (cancelError: any) {
              console.error('[MEMBERSHIP-SUCCESS] ⚠️ Failed to cancel Stripe subscription (non-fatal):', {
                subscriptionId: stripeSub.id,
          ...
          console.log('[MEMBERSHIP-SUCCESS] ✅ No active Stripe subscriptions found for customer - safe to create new one');
        }
      } catch (stripeCheckError: any) {
        console.error('[MEMBERSHIP-SUCCESS] ⚠️ Error checking Stripe API for existing subscriptions (non-fatal):', {
          error: stripeCheckError.message,
          errorCode: stripeCheckError.code,
          customerId: stripeCustomerId,
        });
        // Continue - will still create new subscription
      }
