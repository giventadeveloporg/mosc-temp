...
const BASE_PATH = '/mosc-redesign/calendar';
...
export default function LiturgyCalendarClient({
  ...
  return (
    <div>
      ...
      {loading ? (
      ...
      ) : items.length === 0 && displayView === 'month' ? (
        <div className="inline-flex items-center gap-2 px-4 py-3 bg-orange-50 border-2 border-orange-300 rounded-lg shadow-sm mb-4">
          ...
          <span className="text-sm text-orange-700 font-syro-primary">
            No liturgy days found for this month. Ensure entries exist in Strapi for these dates and
            are linked to your tenant (re-save in Strapi admin if tenant was not auto-assigned).
          </span>
        </div>
      ) : null}
