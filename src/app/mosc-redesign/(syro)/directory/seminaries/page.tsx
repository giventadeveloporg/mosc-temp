...
import { getSeminariesData } from './getSeminariesData';
import type { SeminaryEntry } from './types';
...
const PAGE_SIZE = 20;
const TITLE = 'Seminaries';
const BASE_PATH = '/mosc-redesign/directory/seminaries';
...
export default async function Page({
  ...
  const { entries: allEntries } = await getSeminariesData();
  ...
  const subtitle = hasSearch
    ...
    : `${total} entr${total !== 1 ? 'ies' : 'y'}. Data from the seminaries CMS.`;
          ...
          {entries.length === 0 ? (
            <div className="bg-white rounded-lg p-8 text-center border-l-4 border-syro-red shadow-[rgba(50,50,93,0.25)_0px_6px_12px_-2px,rgba(0,0,0,0.3)_0px_3px_7px_-3px]">
              <p className="font-body text-syro-dark-gray">
                No entries in this section yet. Data is loaded from the seminaries CMS.
              </p>
              <Link
...
function EntryCard({ entry }: { entry: SeminaryEntry }) {
  return (
