...
import { getSeminaryBySlug } from '../getSeminariesData';
import SyroPageBanner from '../../../components/SyroPageBanner';

export const dynamic = 'force-dynamic';

const LIST_PATH = '/mosc-redesign/directory/seminaries';
const TYPE_LABEL = 'Seminaries';
...
/** Extract a usable mailto address from values like "name@x.com (PRO)". */
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const entry = await getSeminaryBySlug(slug);
...
export default async function SeminaryDirectoryDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const entry = await getSeminaryBySlug(slug);
  if (!entry) notFound();
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              ...
              {hasContact ? (
                ...
                <p className="font-body text-syro-dark-gray">
                  No address or contact details are available for this seminary yet.
                </p>
              )}
            </div>
          </div>
