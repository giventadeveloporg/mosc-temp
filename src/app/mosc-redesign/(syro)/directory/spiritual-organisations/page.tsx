...
import type { SpiritualOrganisationEntry } from '../../spiritual-organizations-cms/types';
...
const PAGE_SIZE = 20;
const TITLE = 'Spiritual Organisations';
const BASE_PATH = '/mosc-redesign/directory/spiritual-organisations';
...
export default async function Page({
  ...
  const { entries: allEntries } = await getSpiritualOrganisationsData();
  ...
  const subtitle = hasSearch
    ...
    : `${total} entr${total !== 1 ? 'ies' : 'y'}. Data from the directory API.`;
...
function EntryCard({ entry }: { entry: SpiritualOrganisationEntry }) {
  return (
    <li className="h-full bg-white rounded-lg overflow-hidden sacred-shadow-sm border-l-4 border-syro-red hover:sacred-shadow reverent-transition shadow-[rgba(50,50,93,0.25)_0px_6px_12px_-2px,rgba(0,0,0,0.3)_0px_3px_7px_-3px]">
      <Link
        ...
        className="flex flex-col gap-4 p-6 group h-full"
      >
