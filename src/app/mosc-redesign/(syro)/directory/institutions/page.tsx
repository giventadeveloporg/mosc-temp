...
import { getInstitutionsData } from '../../institutions-cms/getInstitutionsData';
import type { InstitutionEntry } from '../../institutions-cms/types';
...
const BASE_PATH = '/mosc-redesign/directory/institutions';
...
export default async function Page({
  ...
  const { entries: allEntries } = await getInstitutionsData();
  ...
  const subtitle = hasSearch
    ...
    : `${total} entr${total !== 1 ? 'ies' : 'y'}. Data from the institutions CMS.`;
          ...
          {entries.length === 0 ? (
            <div className="bg-white rounded-lg p-8 text-center border-l-4 border-syro-red shadow-[rgba(50,50,93,0.25)_0px_6px_12px_-2px,rgba(0,0,0,0.3)_0px_3px_7px_-3px]">
              <p className="font-body text-syro-dark-gray">
                No entries in this section yet. Data is loaded from the institutions CMS.
              </p>
              <Link
...
function EntryCard({ entry }: { entry: InstitutionEntry }) {
  return (
