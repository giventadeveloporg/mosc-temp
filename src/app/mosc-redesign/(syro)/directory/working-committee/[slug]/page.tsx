...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const entry = await getWorkingCommitteeBySlug(slug);
  if (!entry) return { title: 'Entry Not Found | Directory | MOSC' };
  return {
    title: `${entry.name} | Directory | Malankara Orthodox Syrian Church`,
    description: entry.description ?? `Working Committee member: ${entry.name}.`,
  };
}

export default async function WorkingCommitteeDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const entry = await getWorkingCommitteeBySlug(slug);
  if (!entry) notFound();
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              ...
              {hasContact ? (
                ...
                <p className="font-body text-syro-dark-gray">
                  No address or contact details are available for this member yet.
                </p>
              )}
            </div>
          </div>
