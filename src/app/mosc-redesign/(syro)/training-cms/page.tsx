...
export default async function TrainingCmsPage() {
  const { entries } = await getTrainingProgramsData();
      ...
      <section className="py-16 bg-syro-bg-gray">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {entries.map((program) => {
              const imageSrc =
                program.imageUrl ?? DEFAULT_IMAGE_BY_SLUG[program.slug] ?? '/images/training/sruti.jpg';
              const href = `/mosc-redesign/training-cms/${program.slug}`;
