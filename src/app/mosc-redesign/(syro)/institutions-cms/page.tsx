...
export default async function InstitutionsCmsPage({
  ...
  const cards = INSTITUTION_HUB_CATEGORIES.map((category) => {
    const categoryEntries = filterInstitutionsByCategory(allEntries, category.slug);
    return {
      slug: category.slug,
      title: category.title,
      description: buildCategoryCardDescription(categoryEntries, category),
      imageSrc: pickCategoryCardImage(categoryEntries, category),
  ...
  const pageCards = filtered.slice(start, start + DIRECTORY_PAGE_SIZE);

  const subtitle = hasSearch
    ? `${total} categor${total !== 1 ? 'ies' : 'y'} matching "${searchTerm}".`
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <LiveUrlSearch
              ...
              ariaLabel="Search institution categories by title"
          ...
          {pageCards.length === 0 ? (
            <p className="font-syro-primary text-syro-dark-gray mb-12">
              {hasSearch
                ...
                : 'No institution categories are available at this time.'}
            </p>
          ) : (
            <>
          ...
          {!hasSearch ? <MissionAndStats /> : null}
    ...
    <>
      <div className="max-w-3xl mx-auto mt-16">
        ...
        <p className="font-syro-primary text-lg text-syro-dark-gray leading-relaxed mb-6">
          Following the example of Christ who came to serve, the Malankara Orthodox Syrian Church has
          established numerous institutions dedicated to education, healthcare, and social welfare.
        </p>
        <p className="font-syro-primary text-lg text-syro-dark-gray leading-relaxed">
          From schools and colleges to hospitals and orphanages, from monasteries to medical missions,
          these institutions embody our commitment to serving humanity with love and compassion.
        </p>
      </div>
