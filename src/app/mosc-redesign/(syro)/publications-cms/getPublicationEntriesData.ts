...
import type {
  PublicationEntry,
  PublicationEntriesListOptions,
  PublicationEntriesListResult,
} from './types';
...
const EMPTY_LIST: PublicationEntriesListResult = {
  entries: [],
...
/**
 * Fetches publication entries for the current tenant.
 * Pass page/pageSize/nameSearch for paginated list hubs; omit for load-all (detail fallbacks).
 */
export async function getPublicationEntriesData(
  options?: PublicationEntriesListOptions
): Promise<PublicationEntriesListResult> {
...
export async function getPublicationEntryBySlug(slug: string): Promise<PublicationEntry | null> {
  ...
  return fetchStrapiEntryBySlug({
    ...
    populate: ['image'],
    parse: parseEntry,
    fetchList: async () => (await getPublicationEntriesData({ loadAll: true })).entries,
