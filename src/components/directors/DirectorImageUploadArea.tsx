...
import { uploadDirectorImageServer } from '@/app/admin/event-program-directors/ApiServerActions';

interface DirectorImageUploadAreaProps {
  directorId: number;
...
export default function DirectorImageUploadArea({
  directorId,
  currentImageUrl,
  onUploadSuccess,
  disabled = false,
}: DirectorImageUploadAreaProps) {
  ...
  const handleUpload = async () => {
    if (!selectedFile || !directorId || disabled) return;
    ...
    try {
      await uploadDirectorImageServer(
        directorId,
        selectedFile
      );
    ...
    <div>
      ...
      {currentImageUrl && !selectedFile && (
        <div className="mb-2">
          ...
          />
          <p className="text-xs text-muted-foreground mt-1">Primary photo</p>
        </div>
      )}
      ...
      <p className="text-xs text-muted-foreground mt-2">
        Upload multiple photo images. All images will appear in the Overview section.
      </p>
    </div>
  );
}
