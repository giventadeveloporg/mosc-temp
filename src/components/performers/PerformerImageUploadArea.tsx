...
import { uploadPerformerImageServer } from '@/app/admin/event-featured-performers/ApiServerActions';

interface PerformerImageUploadAreaProps {
  performerId: number;
...
export default function PerformerImageUploadArea({
  performerId,
  imageType,
  currentImageUrl,
  onUploadSuccess,
  disabled = false,
}: PerformerImageUploadAreaProps) {
  ...
  const imageTypeLabel = imageType === 'portrait' ? 'Portrait' : 'Performance';
  ...
  const handleUpload = async () => {
    if (!selectedFile || !performerId || disabled) return;
    ...
    try {
      await uploadPerformerImageServer(
        performerId,
        selectedFile,
        imageType
      );
