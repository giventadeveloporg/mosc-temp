...
import { fetchEventPerformerMediaServer, updateMediaPriorityRankingServer, deleteEventMediaServer } from '@/app/admin/events/[id]/performers/ApiServerActions';
import PriorityRankingEditor from '../sponsors/PriorityRankingEditor';
import ErrorDialog from '@/components/ErrorDialog';

interface EventPerformerMediaGalleryProps {
  eventId: number;
  performerId: number;
...
export default function EventPerformerMediaGallery({
  eventId,
  performerId,
  onMediaSelect,
  onMediaDelete,
  ...
  allowUpload = false,
  onUploadClick,
}: EventPerformerMediaGalleryProps) {
  ...
  useEffect(() => {
    const loadMedia = async () => {
      ...
      setLoading(true);
      try {
        const media = await fetchEventPerformerMediaServer(eventId, performerId);
        setMediaList(media);
      } catch (error) {
        console.error('Failed to fetch event-performer media:', error);
  ...
  if (mediaList.length === 0) {
    return (
      <div className="text-center p-8 bg-card rounded-lg border border-border">
        ...
        <p className="font-body text-muted-foreground mb-4">No media files found for this event-performer combination.</p>
