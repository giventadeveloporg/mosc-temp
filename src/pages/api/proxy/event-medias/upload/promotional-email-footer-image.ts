...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const title =
      firstQueryValue(req.query.title) || 'Promotional Email Footer Image';
    const description =
      firstQueryValue(req.query.description) || 'Promotional email footer image';
    ...
    const apiUrl = `${API_BASE_URL}/api/event-medias/upload/promotional-email-footer-image?${queryParams.toString()}`;
